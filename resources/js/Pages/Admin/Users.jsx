...
export default function Users({ users, stats, filters }) {
    ...
    // Role/Status/Verification now apply the moment you pick them (see
    // UserFiltersMenu/FiltersMenu's immediate-apply mode) instead of
    // waiting for a separate "Apply Filters" click. Each takes the
    // just-picked value directly, since setState hasn't re-rendered yet at
    // the point the request needs to fire.
    const applyRoleFilter = (value) => {
        setRoleFilter(value);
        router.get(route('admin.users'), cleanParams({ search, role: value, status: statusFilter, verified: verifiedFilter, per_page: perPage, sort, direction }, FILTER_DEFAULTS), { preserveState: true, preserveScroll: true });
    };
    const applyStatusFilter = (value) => {
        setStatusFilter(value);
        router.get(route('admin.users'), cleanParams({ search, role: roleFilter, status: value, verified: verifiedFilter, per_page: perPage, sort, direction }, FILTER_DEFAULTS), { preserveState: true, preserveScroll: true });
    };
    const applyVerifiedFilter = (value) => {
        setVerifiedFilter(value);
        router.get(route('admin.users'), cleanParams({ search, role: roleFilter, status: statusFilter, verified: value, per_page: perPage, sort, direction }, FILTER_DEFAULTS), { preserveState: true, preserveScroll: true });
    };
                    ...
                    <div>
                        <div className="mb-2 flex flex-wrap items-center gap-3">
                            ...
                            <UserFiltersMenu
                                ...
                                setVerifiedFilter={applyVerifiedFilter}
