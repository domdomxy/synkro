...
export function describeLog(log) {
    ...
    const taskTitle = d.task_title ? `**${d.task_title}**` : d.task_title;
    const oldAssignee = d.old_assignee ? `**${d.old_assignee}**` : 'unassigned';
    const newAssignee = d.new_assignee ? `**${d.new_assignee}**` : 'unassigned';
    const oldItemTitle = d.old_item_title ? `**${d.old_item_title}**` : d.old_item_title;
    const newItemTitle = d.new_item_title ? `**${d.new_item_title}**` : d.new_item_title;
    const itemTitle = d.item_title ? `**${d.item_title}**` : d.item_title;
    const dependsOnTitle = d.depends_on_title ? `**${d.depends_on_title}**` : d.depends_on_title;
    const fileName = d.name ? `**${d.name}**` : d.name;
    const oldFileName = d.old_name ? `**${d.old_name}**` : d.old_name;
    ...
    switch (log.action) {
        ...
        case 'task_created': return `${actor} created task ${taskTitle}`;
        case 'task_deleted': return `${actor} deleted task ${taskTitle}`;
        case 'task_restored': return `${actor} restored task ${taskTitle} from the trash`;
        case 'task_assigned': return `${actor} assigned ${taskTitle} to ${target}`;
        case 'task_reassigned': return `${actor} reassigned ${taskTitle} from ${oldAssignee} to ${newAssignee}`;
        case 'task_unassigned': return `${actor} unassigned ${taskTitle} (was ${d.old_assignee ? `**${d.old_assignee}**` : d.old_assignee})`;
        case 'task_updated': return `${actor} updated ${taskTitle}`;
        case 'task_started': return `${actor} started ${taskTitle}`;
        case 'task_review_started': return `${actor} started reviewing ${taskTitle}`;
        case 'task_approved': return `${actor} approved ${taskTitle}`;
        case 'task_rejected': return `${actor} sent ${taskTitle} back for changes`;
        case 'submission_reset': return `${actor} reset the submission for ${taskTitle}`;
        case 'submission_kept': return `${actor} kept the submission for ${taskTitle}`;
        case 'task_reopened': return `${actor} reopened ${taskTitle} for changes`;
        case 'checklist_item_updated': return `${actor} edited a checklist item on ${taskTitle} from ${oldItemTitle} to ${newItemTitle}`;
        case 'checklist_item_deleted': return `${actor} removed the checklist item ${itemTitle} from ${taskTitle}`;
        case 'dependency_added': return `${actor} made ${taskTitle} depend on ${dependsOnTitle}`;
        case 'dependency_removed': return `${actor} removed the dependency of ${taskTitle} on ${dependsOnTitle}`;
